...
public class UserSignUpController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    // 요청 파라미터(아이디, 비밀번호, 이메일) 분석
	    String username = request.getParameter("username");
	    String password = request.getParameter("password");
	    String email = request.getParameter("email");
	    
	    // Service layer에 전달할 객체
	    UserSignUpDto dto =UserSignUpDto.builder().username(username).password(password).email(email).build();
	    ...
	    } else { // Post - Redirect - Get  회원가입 실패
	        response.sendRedirect("/jsp02/user/signup?duplicate=true"); // 회원 가입 페이지로 이동
	    }
