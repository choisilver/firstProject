...
public class UserSignInController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 로그인 사용자 아이디를 세션에 저장
        session.setAttribute("signInUser", user.getUsername()); // User객체를 넘겨도 됨. "signInUser" home.jsp, list.jsp에서 if문으로 no empty 를 사용해서,,!
        // EL scope: pageScope -> requestScope -> sessionScope -> applicationScope  순서대로 찾아봄
        // request.setAttribute는 Redirect 방식으로는 값이 들어가질 못함.
