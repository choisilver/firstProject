...
// 로그인이 되어 있으면 요청을 계속해서 처리(Selvlet으로 요청을 전달)
// 로그인이 안되어 있으면 servlet으로 보내지 않고 로그인 창으로 보냄.
public class AuthenticationFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        // 로그인 되어 있는지 체크. - 세션에 로그인 정보(signInUser)가 있는 지를 확인.
        HttpSession session = ((HttpServletRequest) request).getSession(); // 타입 변환을 해야함. (Servlet 상위, Http는 하위)
